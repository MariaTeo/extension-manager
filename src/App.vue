...
const modalActionExtensionId = ref<string | null>(null)
...
const handleRemoveExtension = (id: string) => {
  ...
  pendingActionType.value = 'remove'
  modalTitle.value = `Remove ${extension.name}?`
  modalMessage.value = `Are you sure you want to remove the "${extension.name}" extension? This action cannot be undone.`
  modalConfirmText.value = 'Remove'
  modalCancelText.value = 'Cancel'
  showConfirmModal.value = true
}

const confirmModalAction = async () => {
  if (!modalActionExtensionId.value || !pendingActionType.value) return

  if (pendingActionType.value === 'toggle') {
    const extensionIndex = extensions.value.findIndex(
      (ext) => ext.id === modalActionExtensionId.value,
    )
    if (extensionIndex === -1) return

    const currentIsActive = extensions.value[extensionIndex].isActive
    ...
    try {
      const { error } = await supabase
        ...
        .eq('id', modalActionExtensionId.value)

      if (error) throw error

      extensions.value[extensionIndex].isActive = newIsActive
    } catch (error: any) {
      console.error('Error toggling extension status:', error.message)
    }
  } else if (pendingActionType.value === 'remove') {
    try {
      const { error } = await supabase
        .from('extensions')
        .delete()
      ...
      extensions.value = extensions.value.filter((ext) => ext.id !== modalActionExtensionId.value)
    } catch (error: any) {
...
const resetModal = () => {
  ...
  modalActionExtensionId.value = null
...
<template>
  <div :class="['app-container', theme]">
    <header class="app-header">
      ...
      <ThemeToggle @toggle="toggleTheme" :theme="theme" />
    </header>
      ...
      <section class="extension-list-section">
        <div class="extension-grid">
          <ExtensionItem
            ...
            :key="ext.id"
            :extension="ext"
            @toggle-active-status="handleToggleActiveStatus"
            @remove-extension="handleRemoveExtension"
          />
        </div>
      </section>
    </main>
